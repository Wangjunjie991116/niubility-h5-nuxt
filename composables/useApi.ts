...
//	if (!$axios) {
//		throw new Error('Axios instance not found. Make sure axios plugin is properly configured.');
//	}
...
//		return $axios.post<T>(url, data, config).then((response) => response.data as T);
//	};
...
//		return $axios.put<T>(url, data, config).then((response) => response.data as T);
//	};
...
//	 * PATCH 请求
//	 */
//	const patch = <T = any>(url: string, data?: any, config?: RequestConfig): Promise<T> => {
//		return $axios.patch<T>(url, data, config).then((response) => response.data as T);
//	};
